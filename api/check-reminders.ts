  ...
  try {
    ...
    for (const lead of leads) {
      ...
      for (const minutes of notifyBeforeList) {
        // Уведомляем, если:
        // 1. Оставшееся время меньше или равно времени напоминания (например, осталось 28 минут при установке "за 30 минут")
        // 2. Оставшееся время больше 0 (шаг еще не наступил)
        // 3. Разница не слишком старая (чтобы не спамить о старых забытых напоминаниях, например, если cron долго не запускался, ограничим 10 минутами просрочки)
        // 4. Уведомление на этот интервал еще не отправлялось
        const shouldNotify = 
          diffMinutes <= minutes && 
          diffMinutes > -10 && 

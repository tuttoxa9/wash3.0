...
// Функция для расчета почасовой зарплаты (с 9:00 до 21:00 = 12 часов)
export function calculateHourlyWage(dailySalary: number): number {
  return dailySalary / 12; // 12 часов в рабочем дне
}
...
export function generateDailyReportDocx(report: DailyReport, employees: Employee[], date: string) {
  ...
  reports.forEach(({ date, report }) => {
    ...
    // Расчет ЗП с использованием настроек из localStorage
    const totalRevenue = report.totalCash + report.totalNonCash;

    // Определяем метод расчета из localStorage
    const savedSalaryMethod = localStorage.getItem('salaryCalculationMethod') || 'minimumWithPercentage';
    const minimumSettings = JSON.parse(localStorage.getItem('minimumPaymentSettings') || '{}');

    let salary = 0;
    if (savedSalaryMethod === 'minimumWithPercentage') {
      // Используем метод минимальной оплаты + процент из настроек
      // Базовый расчет для мойщика (если роли не определены)
      const percentage = minimumSettings.percentageWasher || 10;
      const minimumPayment = minimumSettings.minimumPaymentWasher || 0;

      const percentageEarnings = totalRevenue * (percentage / 100);
      salary = Math.max(percentageEarnings, minimumPayment);
    } else {
      // Fallback к старому методу, если не выбран метод из настроек
  ...
  let salaryMethodDescription: string;
  if (savedSalaryMethod === 'minimumWithPercentage') {
    const percentage = minimumSettings.percentageWasher || 10;
    const minimumPayment = minimumSettings.minimumPaymentWasher || 0;
    salaryMethodDescription = `${percentage}% от выручки (мин. ${minimumPayment} BYN)`;
  } else {
    salaryMethodDescription = 'Минимальная оплата + процент';
  }

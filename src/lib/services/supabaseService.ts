...
export const carWashService = {
  async add(record: Omit<CarWashRecord, "id">): Promise<CarWashRecord | null> {
    const payload = {
      ...
      payment_method: { 
        ...
        individualSalaries: record.individualSalaries
      }, // JSONB
    ...
    return {
      ...
      individualSalaries: data.payment_method?.individualSalaries,
  ...
  async getByDate(date: string): Promise<CarWashRecord[]> {
    ...
    return (data || []).map((r: any) => ({
      ...
      individualSalaries: r.payment_method?.individualSalaries,

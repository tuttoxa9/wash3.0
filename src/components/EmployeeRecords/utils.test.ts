...
describe("EmployeeRecords utils", () => {
  ...
  describe("getPaymentMethodLabel", () => {
    ...
    it("should return organization name when method is 'organization' and id is found", () => {
      ...
      expect(
        getPaymentMethodLabel("organization", mockOrganizations, "org2"),
      ).toBe("Organization 2");
    });

    it("should return 'Организация' when method is 'organization' but id is not found", () => {
      expect(
        getPaymentMethodLabel("organization", mockOrganizations, "unknown"),
      ).toBe("Организация");
    });

    it("should return 'Организация' when method is 'organization' and id is missing", () => {
      expect(getPaymentMethodLabel("organization", mockOrganizations)).toBe(
        "Организация",
      );
    });

    it("should return the method string itself for unknown methods", () => {
      expect(getPaymentMethodLabel("unknown_method", mockOrganizations)).toBe(
        "unknown_method",
      );
    });
  });
  ...
  describe("getPaymentMethodColor", () => {
    describe("dark theme", () => {
      it("should return correct colors for all methods in dark theme", () => {
        expect(getPaymentMethodColor("cash", "dark")).toBe(
          "text-green-300 bg-green-500/10 border-green-500/20",
        );
        expect(getPaymentMethodColor("card", "dark")).toBe(
          "text-blue-300 bg-blue-500/10 border-blue-500/20",
        );
        expect(getPaymentMethodColor("organization", "dark")).toBe(
    ...
    describe("black theme", () => {
      it("should return correct colors for all methods in black theme", () => {
        expect(getPaymentMethodColor("cash", "black")).toBe(
          "text-green-400 bg-green-500/5 border-green-500/30",
        );
        expect(getPaymentMethodColor("card", "black")).toBe(
          "text-blue-400 bg-blue-500/5 border-blue-500/30",
        );
        expect(getPaymentMethodColor("organization", "black")).toBe(
          "text-purple-400 bg-purple-500/5 border-purple-400/30",
        );
    ...
    describe("default/light theme", () => {
      it("should return correct colors for all methods in light theme", () => {
        expect(getPaymentMethodColor("cash", "light")).toBe(
          "text-green-600 bg-green-50 border-green-200",
        );
        expect(getPaymentMethodColor("card", "light")).toBe(
          "text-blue-600 bg-blue-50 border-blue-200",
        );
        expect(getPaymentMethodColor("organization", "light")).toBe(

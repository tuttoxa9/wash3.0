...
const DailyReportModal: React.FC<DailyReportModalProps> = ({
  ...
  const getPaymentMethodDisplay = (method: PaymentMethod): React.ReactNode => {
    ...
    if (method.type === "debt") {
      if (method.isClosed) {
        const actualMethodText =
          method.actualMethod === "cash"
            ? "Наличные"
            : method.actualMethod === "card"
              ? "Карта"
              : method.actualMethod === "organization"
                ? "Безнал"
                : "Неизвестно";
        return (
    ...
    try {
      ...
      if (updatedRecord) {
        ...
        if (updatedReport?.records) {
          ...
          // Пересчитываем итоги
          const { totalCash, totalCard } = recalculateReportTotals({
  ...
  return (
    <Drawer.Root open={true} onOpenChange={(open) => !open && onClose()}>
      <Drawer.Portal>
        <Drawer.Overlay className="fixed inset-0 bg-black/40 dark:bg-black/60 backdrop-blur-sm z-[100]" />

        <Drawer.Content className="bg-background border border-border/50 flex flex-col rounded-t-[32px] mt-24 max-h-[96vh] fixed bottom-0 left-0 right-0 z-[101] outline-none shadow-2xl">
          <div className="p-4 bg-card rounded-t-[32px] shrink-0 flex justify-center items-center cursor-grab active:cursor-grabbing">
            <div className="w-12 h-1.5 bg-muted rounded-full" />
          </div>

          <div className="p-3 sm:p-4 md:p-6 lg:flex lg:flex-col lg:h-full overflow-y-auto pb-safe">
                      ...
                      {currentReport.cashModifications &&
                        currentReport.cashModifications.filter(
                          ...
                          <span className="text-[9px] font-normal text-muted-foreground mt-0.5">
                            по услугам {currentReport.totalCash.toFixed(2)}
                          </span>
                        )}
                    </div>
                  </div>
                  ...
                  >
                    ...
                    <div className="text-xs sm:text-sm md:text-base font-bold text-card-foreground leading-tight break-words flex flex-col items-center">
                      {(() => {
                        ...
                        const cardMods = (currentReport.cashModifications || [])
                  ...
                  {(() => {
                    const totalCertificate =
                      currentReport.records?.reduce(
                        (sum, record) =>
                          sum +
                          (record.paymentMethod.type === "certificate"
                            ? record.price
                            : 0),
                        0,
                      ) || 0;

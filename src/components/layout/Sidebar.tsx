...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  const uncalculatedCount = currentReport?.records?.filter((record) => {
    ...
    <div className="flex flex-col h-full relative w-full">
      ...
      <nav className="flex-1 space-y-2 overflow-y-auto overflow-x-hidden min-h-0 custom-scrollbar pr-1">
        ...
        >
          ...
          {uncalculatedCount > 0 && (
            <span className="flex h-5 w-5 items-center justify-center rounded-full bg-red-500 text-[10px] font-bold text-white animate-pulse shrink-0">
              {uncalculatedCount}
            </span>
          )}
        </NavLink>
      ...
      <div className="mt-auto pt-4 border-t border-border/60 space-y-4 shrink-0">
        <div>
  ...
  return (
    <>
      ...
      {/* Mobile Sidebar (Vaul Drawer) */}
      <div className="md:hidden">
        <Drawer.Root
          open={isMobileOpen}
          onOpenChange={(open) => {
             if (!open && isMobileOpen) toggleMobileSidebar();
          }}
          shouldScaleBackground
        >
          <Drawer.Portal>
            <Drawer.Overlay className="fixed inset-0 bg-black/40 z-[45] md:hidden" />
            <Drawer.Content
              className="bg-[hsl(var(--sidebar-background))] text-[hsl(var(--sidebar-foreground))] flex flex-col rounded-t-[20px] fixed bottom-0 left-0 right-0 z-[55] outline-none max-h-[88dvh] md:hidden"
            >
              {/* Drag handle */}
              <div className="flex justify-center pt-2.5 pb-2 shrink-0">
                <div className="w-12 h-1.5 rounded-full bg-foreground/20" />
              </div>
              <div className="flex flex-1 overflow-y-auto w-full px-4 pb-6">
                {sidebarContent}
              </div>
            </Drawer.Content>
          </Drawer.Portal>

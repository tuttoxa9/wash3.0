...
import { Building, Calendar, FileDown, Loader2, X } from "lucide-react";
import type React from "react";
import { useEffect, useState } from "react";

type OrganizationsReportProps = {};

const OkleykaOrganizationsReport: React.FC<OrganizationsReportProps> = () => {
  ...
  const [selectedOrganizationId, setSelectedOrganizationId] = useState<
    string | null
  >(null);
  const [records, setRecords] = useState<OkleykaOrder[]>([]);
  ...
  useEffect(() => {
    const loadOrganizationRecords = async () => {
      ...
      setLoading(true);
      try {
        const fetchedRecords = await okleykaOrderService.getByOrganization(
        ...
        const filteredRecords = fetchedRecords.filter((record) => {
          try {
            const recordDate =
              typeof record.date_start === "string"
                ? parseISO(record.date_start)
                : new Date(record.date_start);

            // Проверяем валидность даты
            if (isNaN(recordDate.getTime())) {
              console.warn("Невалидная дата в записи:", record.id, record.date_start);
              return false;
            }

            return recordDate >= startDate && recordDate <= endDate;
          } catch (error) {
            console.error("Ошибка обработки даты:", error, record.date_start);
            return false;
          }
        });
  ...
  const totalAmount = records.reduce((sum, record) => sum + record.totalPrice, 0);
  ...
  const exportToWord = async () => {
    ...
    try {
      ...
      const organizationName =
        state.organizations.find((org) => org.id === selectedOrganizationId)
          ?.name || "Организация";
      ...
      records.forEach((record, index) => {
        const recordDate =
          typeof record.date_start === "string"
            ? format(parseISO(record.date_start), "dd.MM.yyyy", { locale: ru })
            : format(new Date(record.date_start), "dd.MM.yyyy", { locale: ru });
          ...
          new TableRow({
            children: [
              ...
              new TableCell({
                children: [
                  new Paragraph({
                    text: record.totalPrice.toFixed(2),
        ...
        sections: [
          {
            ...
            children: [
              ...
              new Paragraph({
                ...
                children: [
                  new TextRun({
                    text: `Итого: ${totalAmount.toFixed(2)} BYN`,
                    bold: true,
                  }),
                ],
              }),
            ...
            {state.organizations.length > 0 ? (
              ...
              <div className="text-center py-4 text-muted-foreground">
                ...
                <p className="text-sm mt-1">
                  Добавьте организации в разделе настроек
                </p>
              </div>
            )}
          </div>
          ...
          {loading ? (
            ...
            <div className="overflow-x-auto custom-scrollbar mobile-table">
              ...
              <div className="mt-4 text-xs text-muted-foreground">
                <p>
                  Отчет сформирован: {format(new Date(), "dd.MM.yyyy HH:mm:ss")}
                </p>
              </div>
            </div>
          ) : (
            <div className="text-center py-8 text-muted-foreground">
              ...
              <p className="text-sm mt-1">
                Попробуйте изменить период отчета или выбрать другую организацию
              </p>
            </div>
          )}
        </div>
...
export default OkleykaOrganizationsReport;

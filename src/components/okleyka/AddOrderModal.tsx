...
interface AddOrderModalProps {
  ...
  onCreated: (order: OkleykaOrder) => void;
}
...
const AddOrderModal: React.FC<AddOrderModalProps> = ({
  ...
  const checkAvailability = useCallback(
    async (box: 1 | 2, start: string, end: string) => {
      if (!start || !end || start > end) return;
      setCheckingAvailability(true);
      try {
        const result = await okleykaOrderService.checkBoxAvailability(box, start, end);
  ...
  useEffect(() => {
    ...
    checkTimer.current = setTimeout(() => {
      checkAvailability(boxNumber, dateStart, dateEnd);
    }, 400);
    return () => {
      if (checkTimer.current) clearTimeout(checkTimer.current);
    };
  }, [boxNumber, dateStart, dateEnd, checkAvailability]);
  ...
  const doCreate = async () => {
    ...
    try {
      ...
      const created = await okleykaOrderService.create({
        boxNumber,
        dateStart,
        dateEnd,
      ...
      console.error("[AddOrderModal] doCreate error:", err);
          ...
          <>
            ...
            >
              ...
              <div className="flex items-center justify-between px-5 py-4 border-b border-white/10">
                <div>
                  <h2 className="text-lg font-bold text-white">Новый заказ оклейки</h2>
                  <p className="text-xs text-white/50 mt-0.5">Заполните данные заказа</p>
                </div>
                <button
                  onClick={onClose}
              ...
              {/* Footer */}
              <div className="px-5 py-4 border-t border-white/10 bg-[#0f0f14]">
                <button
                  type="button"
                  onClick={handleSubmit}
                  disabled={submitting}
                  className="w-full py-3.5 rounded-xl bg-gradient-to-r from-violet-600 to-purple-600 hover:from-violet-500 hover:to-purple-500 text-white font-bold text-sm shadow-lg shadow-violet-900/40 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                >
                  {submitting ? (
                    <>
                      <span className="animate-spin">⟳</span>

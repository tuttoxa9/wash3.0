...
import type { OkleykaShift, OkleykaCashModification } from "@/lib/types/okleyka";
import {
  Wallet,
  CheckCircle,
  Users,
...
interface Props {
  shift: OkleykaShift;
  totalServicesCash: number;
  onCloseCash: () => void;
  onPayout: () => void;
}

export default function OkleykaCashStateWidget({
  shift,
  totalServicesCash,
  onCloseCash,
  onPayout,
  ...
  const isCashClosed = !shift.isOpen;

  // Формирование кассы (Грязная касса ДО любых выплат и изъятий ЗП/Сейфа)
  const cashMods = (shift.cashModifications || []).filter(m => !m.method || m.method === "cash");
  ...
  const startCash = shift.startOfDayCash || 0;
  const servicesCash = totalServicesCash || 0;
  ...
  // Расходы из кассы
  const totalPayouts = Object.values(shift.salaryPayouts || {}).reduce((sum, val) => sum + val, 0);

  // Фактическая грязная касса
  const actualGrossCash = shift.actualEndCash !== undefined && shift.actualEndCash !== null ? shift.actualEndCash : 0;

  // Текущий физический остаток в ящике
  const currentPhysicalCash = isCashClosed
    ? actualGrossCash - totalPayouts
    : expectedGrossCash - totalPayouts;
  ...
  const handleAddModification = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const modification: OkleykaCashModification = {
      ...
      const updatedModifications = [...(shift.cashModifications || []), modification];
      const updatedShift = { ...shift, cashModifications: updatedModifications };
  ...
  const handleDeleteModification = async (id: string) => {
    ...
    setLoading(true);
    try {
      const updatedModifications = (shift.cashModifications || []).filter((m) => m.id !== id);
      const updatedShift = { ...shift, cashModifications: updatedModifications };
  ...
  const modifications = shift.cashModifications || [];
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="grid grid-cols-2 gap-2 sm:gap-3">
            ...
            {/* Выплачено ЗП */}
            <div className="bg-muted/10 border border-border/40 rounded-xl p-3 flex flex-col justify-between hover:bg-muted/20 transition-all duration-200 col-span-2">

...
import type { OkleykaEmployee, OkleykaAppointment } from "@/lib/types/okleyka";
import { Skeleton } from "@/components/ui/skeleton";

interface OkleykaPreShiftScreenProps {
  selectedDate: string;
  ...
  setEmployeeRoles: (roles: Record<string, "admin" | "installer">) => void;
  startShift: () => void;
  ...
  upcomingAppointments: OkleykaAppointment[];
  totalAppointmentsToday: number;
}

export const OkleykaPreShiftScreen: React.FC<OkleykaPreShiftScreenProps> = ({
  selectedDate,
  isCalendarOpen,
  setIsCalendarOpen,
  calendarRef,
            ...
            {loading.employees ? (
              ...
              <div className="text-center py-10 bg-accent/30 rounded-xl border border-dashed border-border">
                ...
                <p className="text-sm text-muted-foreground mt-1">
                  Добавьте сотрудников на странице настроек оклейки
                </p>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                {upcomingAppointments.map((appointment) => (
                  <div key={appointment.id} className="p-3 bg-background rounded-xl border border-border/50 flex flex-col gap-1">
                    <div className="flex justify-between items-center">
                      ...
                      <span className="text-xs font-medium px-2 py-0.5 rounded-full bg-primary/10 text-primary truncate max-w-[120px]">
                        {appointment.service}
                      </span>
                    </div>
